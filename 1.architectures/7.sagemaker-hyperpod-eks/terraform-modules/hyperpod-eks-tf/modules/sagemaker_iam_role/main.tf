...
resource "aws_iam_policy" "eks_cni_policy" {
  name = "${var.resource_name_prefix}-EKS_CNI_Policy-${data.aws_region.current.id}"
  path = "/"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
...
resource "aws_iam_policy" "rig_policy" {
  ...
  name = "${var.resource_name_prefix}-RigPolicy-${data.aws_region.current.id}"
  path = "/"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = concat(
...
resource "aws_iam_policy" "subnet_policy" {
  name = "${var.resource_name_prefix}-Subnet_Policy-${data.aws_region.current.id}"
  path = "/"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
...
resource "aws_iam_policy" "sg_policy" {
  name = "${var.resource_name_prefix}-SG_Policy-${data.aws_region.current.id}"
  path = "/"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [

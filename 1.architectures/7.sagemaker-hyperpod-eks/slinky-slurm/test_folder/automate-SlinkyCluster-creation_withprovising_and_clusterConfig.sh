...
setup_lifecycle_scripts() {
    ...
    echo -e "${BLUE}Uploading your lifecycle scripts to S3 bucket ${YELLOW}${S3_BUCKET_NAME}${NC}"
    # upload data
    upload_to_s3() {
        aws s3 cp --recursive base-config/ s3://${S3_BUCKET_NAME}/src --output json
    }
...
EOL
    ...
    # upload data
    upload_to_s3() {
        aws s3 cp provisioning_parameters.json s3://${S3_BUCKET_NAME}/src/ --output json
    }
...
configure_cluster_users() {
    ...
    if [[ "${CONFIGURE_USERS}" == "yes" ]]; then
        ...
        echo -e "${BLUE}Uploading shared_users.txt to S3 bucket: $S3_BUCKET_NAME...${NC}"

        if ! output=$(aws s3 cp shared_users.txt s3://${S3_BUCKET_NAME}/src/ --output json 2>&1); then
            ...
            echo -e "${GREEN}✅ User configuration file uploaded successfully to s3://${S3_BUCKET_NAME}/src/shared_users.txt${NC}"
        fi
    else

...
from torch.sagemaker.distributed.fsdp.fully_sharded_data_parallel import (
    register_tp_processing_pre_comm_hook,
)
...
def main(args):
    ...
    smp_config_dict["tensor_parallel_degree"] = args.tensor_parallel_degree
    if args.tensor_parallel_degree > 1 and args.use_smp_implementation < 1:
        args.use_smp_implementation = 1
        if global_rank == 0:
            logger.info(
                f"Tensor parallelism (TP) is enabled as tensor_parallel_degree is set to {args.tensor_parallel_degree} (>1). Switching use_smp_implementation to 1 so we can use SMP optimized implementation for TP."
            )
    ...
    with tsm_utils.timeit(True, "Model creation", global_rank):
        ...
        if args.use_smp_implementation:
            load_state_dict_from_rank0 = (
                True if finetune_with_pretrained_weights_check(args) else False
            )

...
# Copyright ColossalAI
# Modified from https://github.com/hpcaitech/ColossalAI/blob/main/applications/ColossalChat/examples/training_scripts/lora_finetune.py
...
import torch.distributed.distributed_c10d as d10d
...
def train(args) -> None:
    ...
    # ==============================
    d10d.default_pg_nccl_timeout = timedelta(hours=1)
    ...
    coordinator.print_on_master(f"Load dataset: {args.dataset} split: {args.dataset_split}")
    ...
    dataset = dataset.map(lambda sample: apply_chat_template(tokenizer, sample["messages"]), remove_columns=['problem', 'solution', 'messages'])
    dataset = dataset.filter(lambda sample: len(sample["tokens"]) <= args.max_length)
    ...
    coordinator.print_on_master("Model loading started")
    start_time = time.time()
    booster.load_model(model, args.pretrained, low_cpu_mem_mode=False, num_threads=8)
    coordinator.print_on_master(f"Model loaded in {time.time() - start_time:.2f} seconds")
    ...
    # Final save.
    coordinator.print_on_master(f"Start saving final model checkpoint to {args.save_dir}")
    start_time = time.time()
    if args.lora_rank > 0:
        booster.save_lora_as_pretrained(model, os.path.join(args.save_dir, "lora"))
    else:
        booster.save_model(model, os.path.join(args.save_dir, "modeling"), shard=True)
    coordinator.print_on_master(f"Saved final model checkpoint at epoch {epoch} at folder {args.save_dir} in {time.time() - start_time:.2f} seconds")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset", type=str, required=True, help="Dataset for training.")

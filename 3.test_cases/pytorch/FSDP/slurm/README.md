...
For this example, we'll be using the [wikimedia/wikipedia](https://huggingface.co/datasets/wikimedia/wikipedia). Instead of downloading the whole thing, the `create_streaming_dataloaders` function will stream the dataset from [HuggingFace](https://huggingface.co/datasets), so there's no data prep required for running this training. 

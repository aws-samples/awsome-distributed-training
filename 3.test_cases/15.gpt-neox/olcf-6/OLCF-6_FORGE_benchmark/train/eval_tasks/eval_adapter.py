...
class EvalHarnessAdapter(GPT2LM):
    ...
    def _dp_gather_cpu(self, logits):
        """
        Gather logits from all data parallel ranks
        """
        logits_cpu = logtis.cpu()
        if logits is not None:
            tensor_list = [torch.zeros_like(logits_cpu) for _ in range(self.dp_world_size)]
            torch.distributed.all_gather(
                tensor_list, logits_cpu, group=mpu.get_data_parallel_group()
            )
            logits = torch.cat(tensor_list, dim=0).cuda()
            return logits

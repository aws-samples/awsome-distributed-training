...
def train(
    ...
    train_loss = 0.0
    #context_manager = torch.autograd.profiler.emit_nvtx() if cfg.use_nsight else nullcontext()
    ...
    for batch_idx, batch in enumerate(train_loader, start=start_step + 1):
        if batch_idx == warmup_steps and cfg.use_nsight and rank==0:
            torch.cuda.cudart().cudaProfilerStart()
            print(f"========= NSight profiling intiated at step {warmup_steps} =========")
            #nvtx.range_push("Batch " + str(batch_idx))
        if batch_idx > cfg.num_steps:
            if cfg.use_nsight and rank == 0:
                torch.cuda.cudart.cudaProfilerStop()
                print(f"========= NSight profiling stopped =========")
                break
            break
